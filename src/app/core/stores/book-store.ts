...
export class BookStore {
  ...
  bookDetails = signal<BookDetail | null>(null);
  loadBookDetails = signal(false);
  errorBookDetails = signal<string | null>(null);
  ...
  async loadBookById(id: string): Promise<void> {
    this.loading.set(true);
    this.error.set(null);

    try {
      const result = await this.bookApi.getBookById(id).toPromise();

      if (result?.succeeded) {
        if (result.data) {
          this.bookDetails.set(result.data);
        }
      } else {
        this.error.set(result?.message || 'Failed to load books');
      }
